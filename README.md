...
### 🏗️ 複数現場（プロジェクト）管理 & データバックアップ
- **複数現場（プロジェクト）切替**: 座標データや野帳を現場ごとに分離して個別管理
- **一括バックアップ＆復元 (JSON)**: 全現場・全座標・水準野帳・計算履歴を1クリックでJSONエクスポート＆完全復元
- **PWA (Progressive Web App) 対応**: サービスワーカーによる事前キャッシュで、山間部や地下など電波の届かない現場でも完全オフライン動作

### 📍 座標管理
- **座標一覧表示**: 登録した点の座標を一覧で管理（現場ごとの自動フィルタリングに対応）
...
#### 標高改定対応 (標高補正計算)
- 国土地理院が提供する水準点標高補正パラメータ（.par）による補正
- 双一次内挿法 (Bilinear interpolation) を用いた高精度の算出
- パラメータファイルのブラウザ内永続化（IndexedDB）対応
- 対象ポイントを一括で選択・更新可能

#### ジオイド高計算
- 国土地理院のWeb API連携によるジオイド高の正確な取得
- 楕円体高と標高の相互変換 (Z座標の一括上書き更新機能)
- 緯度経度ベースでの一括検索と取得

#### 標高系変換
- 日本の代表的な標高基準（T.P., O.P., K.P., N.P.）の相互変換
- オフセット値による正確な換算
- Z座標の一括上書き更新対応

#### 幅杭計算（オフセット計算）
- 始点・終点で定義される基準線からの前進距離と左右の幅から座標計算
- トグルによる直感的な左右指定・同時に複数の新点をリスト入力可能
- 結果の一括DB保存
...
### v3.0.0 (2026-08-11)
- **複数現場（プロジェクト）管理機能**: 座標データや野帳を現場ごとに個別管理・整理・切り替えができる機能を追加
- **全データ一括バックアップ＆復元 (JSON)**: 全現場・全座標・水準野帳・計算履歴を一括でJSON保存・復元（統合/置き換え）できる機能を追加
- **PWA (Progressive Web App) オフライン対応**: サービスワーカー構築により山間部や地下などのオフライン現場環境での完全動作に対応
- **Webパフォーマンス最適化**: コード分割（`manualChunks` & `React.lazy`）によりメインJSバンドルを 1.84MB → 542KB (約70%削減) へ超軽量化
- **コード品質・React 19・TypeScript適合**: ESLint全61件のエラー修復、React 19 Hooks規約、TypeScript型安全性を全面強化

### v2.1.0 (2026-04-05)
- 幅杭計算（オフセット計算）機能の追加

### v2.0.0 (2026-02-24)
- 標高系変換機能の追加（T.P., O.P., K.P., N.P.の相互変換）

### v1.9.0 (2026-02-21)
- ジオイド高計算機能の追加 (国土地理院API連携)
- 楕円体高と標高の相互変換に対応

### v1.8.0 (2026-02-21)
- 標高改定対応（補正計算）機能の追加
- 国土地理院パラメータファイル(.par)の読み込みとブラウザ内保存(IndexedDB)
- 緯度経度の度分秒(DMS)表示の標準化
- ユーザーマニュアルとREADMEの更新

### v1.7.0 (2026-02-17)
- 各種エクスポート時にファイル名を入力可能に
- モバイル表示向けにUIとレイアウトを大幅調整
