...
// --- Pure Calculation Logic Helper ---
function calculateLevelingRows(rows: LevelingRow[]) {
    let currentIH: number | null = null;
    let runningGH: number | null = null;
    ...
    return rows.map((row) => {
        ...
        let calcIH: number | undefined = undefined;
        if (r.bs !== undefined && !isNaN(r.bs) && runningGH !== null) {
            calcIH = runningGH + r.bs;
            currentIH = calcIH;
        } else {
            r.ih = undefined;
        }
...
export function LevelingBook() {
    ...
    const calculatedRows = useMemo(() => calculateLevelingRows(rows), [rows]);

    const updateRow = (id: string, field: keyof LevelingRow, value: string | number | boolean | undefined) => {
